...
Where
  ...
  * **options** defines the behavior of the server
    * **options.secure** defines if the connection should use TLS (if `true`) or not (the default). If the server does not start in TLS mode, it is still possible to upgrade clear text socket to TLS socket with the STARTTLS command (unless you disable support for it). If secure is `true`, [additional tls options for tls.createServer](http://nodejs.org/api/tls.html#tls_tls_createserver_options_secureconnectionlistener) can be added directly onto this options object.
